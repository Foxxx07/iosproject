...
CREATE TABLE IF NOT EXISTS `imap_contacts`.`users` (
	`key`			BINARY(8)		NOT NULL			COMMENT 'Identifiant unique d’un utilisateur',
	...
	`password`		BINARY(64)		NOT NULL			COMMENT 'Hash: SHA256',
...
CREATE TABLE IF NOT EXISTS `imap_contacts`.`friends` (
	...
	`user_b`		BINARY(8)	NOT NULL				COMMENT 'Identifiant de l\'utilisateur B',
